...
function worstHitFraction(row: { threat: number; defs: EncounterDef[] }): number {
  const worst = Math.max(...row.defs.flatMap((d) => d.enemies.map((e) => e.damage)));
  const landed = resolveAttack(worst, 0, 0, 1, 0.99, 0).damageDealt;
  return landed / expectedHull(row.threat);
}
...
describe("难度必须随星区威胁上升", () => {
  ...
  it("越往后越耐打,但幅度有限——变强要能被感觉到,又不能变成无敌", () => {
    const first = worstHitFraction(rows[0]);
    const last = worstHitFraction(rows[rows.length - 1]);
    expect(last, "终局的一击占比反而更高了,玩家一路变强的回报没兑现").toBeLessThan(first);
    expect(
      first / last,
      `从头到尾相对威胁掉了 ${(first / last).toFixed(0)} 倍——改前是 16 倍,那正是后期没有输的可能的原因`,
    ).toBeLessThan(4);
  });
});
