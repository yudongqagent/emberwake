...
describe("画面循环", () => {
  ...
  /** 战斗逻辑必须留在自己的定时器上,**不能**被改写成 rAF 驱动。
   *
   * 第 84 轮写这条时的理由是"切走时战斗不该停在那里";第 89 轮把这个判断
   * 推翻了一半——看不见的时候战斗**就该**停(见 visibilityPause.test.ts)。
   * 但那是由一道显式的闸门做的,而不是把心跳绑到帧上。两者的区别很实在:
   * 绑到帧上意味着帧率一低战斗就变慢,而且"暂停"和"卡顿"再也分不开。 */
  it("战斗逻辑仍然走定时器,不是 rAF 驱动", () => {
    expect(COMBAT_SRC, "战斗心跳被改成 rAF 了——帧率会变成战斗速度").toMatch(
      /const id = setInterval\(\(\) => \{[\s\S]{0,1600}combatTick\(\)/,
    );
  });
