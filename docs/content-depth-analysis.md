...
## 余烬刻印:第一条真正无尽的循环 —— 2026-08-30

上面「仍未解决」的两条其实是同一条,已经一起解决。

**裂隙撞墙不是问题**——"能潜多深"作为分数是好设计。问题是**顶到天花板之后再往下
潜什么都不给**:存档里根本没有"最深多少"这个字段,你打破自己的纪录,游戏连记都不记。

`data/sigils.ts` 把深度变成了跨局的永久成长:每次深潜结算刻印,刷新最深纪录另有
一大笔(每突破一层 +2),刻印花在六个抬硬上限的节点上。于是"再深一层"买到的东西
会让下一次能更深。

六个节点每一个都在代码里真的被读——`sigils.test.ts` 会逐个实测。今天已经抓到四个
"显示了却没人读"的东西(16 个死 flag、船员 approval、lockedTraitSlot、
unlockHullClass),一个买得到却没效果的升级是同一种病,而且玩家为它付了代价。

**进阶路线的重玩价值**同样落在这里:刻印不绑定任何一条骨架,所以换一条路线重铸
不会作废你攒下的东西。
